<?php $this->load->view('partials/header') ?>
  ...
  <body>
              ...
              <div class="col-lg-6">
                <div class="card">
                  <div class="card-header">
                    ...
                    <div class="card-options">
                      <a href="jscript:void(0)" onclick="_newLab()" class="btn btn-outline-primary">Tambah</a>
                    </div>
                  </div>
                  <div class="card-body p-0">
                      <ul class="list-group borderless">
                        ...
                        <li class="list-group-item">
                          ...
                          <div class="btn-group pull-right">
                            <button class="btn btn-primary btn-sm" onclick='_editLab(<?= json_encode($value) ?>)'> <i class="fe fe-edit-2"></i> </button>
                            <button class="btn btn-danger btn-sm" onclick="_deleteLab(<?= $value['id'] ?>)"> <i class="fe fe-trash"></i> </button>
                          </div>

                          <div class="media">
                            <div class="media-body">
                              <h4 class="m-0"><?= $value['nama_lab'] ?></h4>
                              <p class="text-muted mb-0"><?= $value['dn'] ?></p>
                              <p class="text-muted mb-0"><?= $value['jadwal_hari'] ?></p>
                              <p class="text-muted mb-0"><?= $value['jam_buka'].' - '.$value['jam_tutup'] ?></p>
                            </div>
                          </div>
    ...
    <?php
      ...
      $this->load->view('pages/dashboard/rumah-sakit/modals/form-laboratorium');
    ?>

    <script>

      $(document).ready(function(){
        $('#form-dokter,#form-lab').find('input').not('input[type=file],input[type=checkbox]').prop('required',true)
      })
      ...
      function _deleteLab(id){
        fn.confirm({
            message: 'Yakin ingin menghapus data laboratorium ini?',
            textConfirm: 'Ya',
            confirm: (e) => {
                fn.request({
                    url: 'lab/delete/'+id,
